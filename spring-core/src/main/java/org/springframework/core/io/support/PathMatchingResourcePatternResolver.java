...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 *  给DefaultResourceLoader 指定classLoader
	 * Create a new PathMatchingResourcePatternResolver with a DefaultResourceLoader.
	 *
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			//  路径是否包含  AntPathMatcher  通配符
			// 类路径资源（可能有多个同名资源）
			// a class path resource (multiple resources for same name possible)
			if (getPathMatcher().isPattern(locationPattern.substring(CLASSPATH_ALL_URL_PREFIX.length()))) {
				// a class path resource pattern
				//根据Ant通配符获取指定路径下所有Resource
				return findPathMatchingResources(locationPattern);
			} else {
				// 没有通配符的情况
				// 根据指定路径返回指定 URLResource
				// 返回当前类载入器路径上的全部反复资源以及父类载入器上的全部反复资源
			...
			if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {
				...
				//不带通配符的情况下获取单个Resource 以数组的方式返回
	...
	/**
	 * 查找指定目录下所有Resource  将URL对象 转换为 URLResource
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			if (rootDirUrl.getProtocol().startsWith(ResourceUtils.URL_PROTOCOL_VFS)) {
				...
				// 根据rootDirResource(URLResource )subPattern 匹配的jar文件
				result.addAll(doFindPathMatchingJarResources(rootDirResource, rootDirUrl, subPattern));
			} else {
				// 找出目录下所有文件 抽象为 FileSystemResource
				//  ServletContextResourcePatternResolver  除了本类默认实现 同时有servlet容器的实现
